...
import { updateJob, getJobDetails } from '@/apis/job';
...
export default function EditJobPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        const loadJob = async () => {
            try {
                ...
                const jobTypeValue = (job.job_type || '').toLowerCase();

                setFormData({
                    title: job.title || '',
                    description: job.description || '',
                    job_type: jobTypeValue || 'full-time',
                    work_type: job.is_remote ? 'Remote' : 'On-site',
                    salary_min: job.salary_min ?? '',
                    salary_max: job.salary_max ?? '',
                    experience_min: job.experience_min ?? 0,
                    experience_max: job.experience_max ?? 1,
                    qualifications: job.qualifications || '',
                    status: job.status || 'published'
                });

                setCityQuery(job.location || '');
                if (job.location_id && job.location) {
                    setSelectedLocation({ id: job.location_id, label: job.location });
                } else {
                    setSelectedLocation(null);
                }

                const details = Array.isArray(job.required_skills_details) ? job.required_skills_details : [];
                if (details.length > 0) {
                    setSkills(details.map((s: any) => ({
                        id: s.skill_id ?? null,
                        name: s.skill_name ?? String(s)
                    })));
                } else if (Array.isArray(job.required_skills)) {
                    setSkills(job.required_skills.map((s: any) => {
                        if (typeof s === 'number') return { id: s, name: String(s) };
                        if (typeof s === 'string') {
                            if (/^\d+$/.test(s)) return { id: Number(s), name: s };
    ...
    const handleSubmit = async (e?: React.FormEvent, statusOverride?: string) => {
        ...
        try {
            ...
            toast.success(statusOverride === 'published' ? 'Job posted successfully!' : 'Job updated successfully!');
            router.push('/employer/jobs');
        } catch (error) {
            const msg = getApiErrorMessage(error, statusOverride === 'published' ? 'Failed to post job.' : 'Failed to update job.');
            toast.error(msg);
        } finally {
            setLoading(false);
        }
    ...
    if (fetching) return <div className="p-8 text-center">Loading job data...</div>;

    return (
        <div className="max-w-4xl mx-auto pb-12">
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-3xl font-extrabold text-gray-900">Edit Job</h1>
                <button
                    type="button"
                    onClick={() => handleSubmit(undefined, 'published')}
                    disabled={loading}
                    className="px-6 py-2.5 rounded-lg bg-red-500 hover:bg-red-600 text-white font-semibold text-sm transition-colors disabled:opacity-50"
                >
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-8 space-y-8">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="px-6 py-2.5 rounded-lg bg-[#5B4DFF] hover:bg-[#4a3dec] text-white font-semibold text-sm transition-colors disabled:opacity-50"
                    >
